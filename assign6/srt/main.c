...
int nthreads;
scene_t scene;
float arr[width][height][3];

void *perform_work(void *tnum) {
	Vec3 camera_pos;
	...
	/* for every pixel */
	//split up the printing for to multiple threads
	for( int px = *(int*)tnum; px<width; px += nthreads )
	{
		...
		for( int py=0; py<height; ++py )
		{
			...
			arr[px][py][0] = scaled_color[0];
			arr[px][py][1] = scaled_color[1];
			arr[px][py][2] = scaled_color[2];
		}
	}
	return NULL;
}
...
main( int argc, char **argv )
{
	...
	{
		tnum[i] = i;
		int creat = pthread_create(&threads[i], NULL, perform_work, &tnum[i]);
		if (creat)
		{
